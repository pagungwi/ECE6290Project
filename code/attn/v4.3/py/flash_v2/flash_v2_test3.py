...
ssum0 = torch.pow(expected_attention - O, 2).sum()
ssum1 = torch.pow(O, 2).sum()
...
print("ref:", O.shape)
print("ref:", O)
print("O:", expected_attention.shape)
print("O:", expected_attention)


Oi_float = expected_attention.view(torch.float).cpu().numpy()
print("save ....")
...
for block_start_Br in range(0, N1, Br):
    ...
    for block_start_Bc in range(0, N1, Bc):
        ...
        pij_hat = torch.exp((Sij - mi_new).to(torch.bfloat16))

        lij_hat = torch.sum(pij_hat, dim=1)[:, None]

        correction = torch.exp((mi - mi_new).to(torch.bfloat16))
